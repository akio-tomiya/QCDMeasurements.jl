// Independent Shamir domain-wall PP/J5q reference for QCDMeasurements.jl.
//
...
// physical wall-wall pseudoscalar correlator, the midpoint J5q correlator,
// and their timeslice ratio used to determine the residual mass.
...
int main(int argc, char **argv)
{
  ...
  for (int spin = 0; spin < Ns; ++spin) {
    for (int colour = 0; colour < Nc; ++colour) {
      ...
      PropToFerm<DomainWallFermionD>(
          source4, source, spin, colour);
      ...
      FermToProp<DomainWallFermionD>(
          propagator, solution4, spin, colour);
      FermToProp<DomainWallFermionD>(
          propagator5, solution5, spin, colour);
    }
  }
